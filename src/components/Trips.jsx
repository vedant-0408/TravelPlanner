...
const columns=[
    {name:"ID" , uid:"id"},
    {name:"NAME" , uid:"name"},
    {name:"CITY" , uid:"destinationItinerary"},
...
export default function Trips() {
  ...
  const renderCell = React.useCallback((trip, columnKey) => {
    ...
    switch (columnKey) {
      case "id":{
        return <div className="text-primary-500"><a href={`/trips/${cellValue}`} target="_blank">{cellValue}</a></div>;
      }
      case "destinationItinerary":{
        const colors=["primary","secondary","success","warning","danger"];
        let currentIndex=0;
        if(Array.isArray(cellValue)){
            const itineraryValues=cellValue.slice(0,4);
            return (
                <div className="flex gap-2">
                {
                    itineraryValues.map(value=><Chip key={value.placeText} color={`${colors[currentIndex++ % colors.length]}`}>{value.placeText}</Chip>)
                }</div>
            )
        }
        return null;
